...
import './ClozeTestModal.css';

const ClozeTestModal = ({ questions, answers }) => {
  ...
  const checkAnswers = () => {
    ...
    userAnswers.forEach((answerPair, index) => {
      if (
        ...
        answerPair[2].trim().toLowerCase() === answers[index][2].toLowerCase() &&
        answerPair[3].trim().toLowerCase() === answers[index][3].toLowerCase()
      ) {
        correctCount++;
      }
    });
  ...
  return (
    <div className="cloze-test-container">
      <h2 style={{ textAlign: 'center', marginBottom: '20px' }}>
        Bài Tập Điền Vào Chỗ Trống Modal
      </h2>
      <div
            ...
            <tr key={index}>
              ...
              <td>
                <div className="input-wrapper" style={{ position: 'relative' }}>
                  <input
                    type="text"
                    value={userAnswers[index][3]}
                    onChange={(e) => handleChange(index, 3, e.target.value)}
                    placeholder="..."
                    className={
                      showResults
                        ? userAnswers[index][3].trim().toLowerCase() ===
                          answers[index][3].toLowerCase()
                          ? 'cloze-test-input correct'
                          : 'cloze-test-input incorrect'
                        : 'cloze-test-input'
                    }
